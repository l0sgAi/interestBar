...
-- 添加列注释
COMMENT ON COLUMN domains.users.id IS 'ID';
...
COMMENT ON COLUMN domains.category.id IS '分类ID';
...
COMMENT ON COLUMN domains.category.parent_id IS '父分类ID(0为顶级)';
...
-- 插入顶级分类数据
INSERT INTO domains.category (name, slug, icon, parent_id, sort, circle_count) VALUES
-- 第一梯队：超高热度/刚需
...
COMMENT ON COLUMN domains.circle.id IS '主键ID';
...
COMMENT ON COLUMN domains.circle.creator_id IS '创建者用户ID';
COMMENT ON COLUMN domains.circle.category_id IS '所属分类ID';
...
COMMENT ON COLUMN domains.post.circle_id IS '所属圈子ID';
COMMENT ON COLUMN domains.post.user_id IS '作者用户ID';
...
-- 添加列注释
COMMENT ON COLUMN domains.comment.id IS '主键ID';
COMMENT ON COLUMN domains.comment.post_id IS '所属帖子ID';
COMMENT ON COLUMN domains.comment.user_id IS '评论发布者ID';
COMMENT ON COLUMN domains.comment.root_id IS '根评论ID：0表示顶层评论，否则存放所属的顶层评论ID';
COMMENT ON COLUMN domains.comment.reply_to_id IS '被回复的评论ID：0表示非回复特定人，否则存放被回复的那条评论ID';
COMMENT ON COLUMN domains.comment.reply_to_user_id IS '被回复的用户id';
...
-- 场景：查询某个根评论下的所有子回复
CREATE INDEX idx_comment_root_id ON domains.comment (root_id) WHERE root_id > 0;
```
...
COMMENT ON COLUMN domains.comment_like.post_id IS '冗余帖子ID，便于查询';
...
COMMENT ON COLUMN domains.post_like.post_id IS '帖子ID，便于查询';

-- --- 索引优化---

-- 1. 【核心】保证每个用户对每个评论只有一条点赞/取消点赞的记录
CREATE UNIQUE INDEX uk_post_like_user_comment ON domains.post_like(user_id, post_id);
