...
CREATE TABLE circle (
    ...
    description VARCHAR(2000) NOT NULL, -- 描述信息
...
CREATE TABLE comment (
    ...
    -- 1. 归属关系
    post_id BIGINT NOT NULL,            -- 所属帖子ID (对应原 car_id)
    user_id BIGINT NOT NULL,            -- 评论发布者ID
    ...
    -- 5. 状态与时间
    status SMALLINT NOT NULL DEFAULT 1,     -- 状态: 1=正常, 2=审核中, 3=折叠/隐藏
...
COMMENT ON COLUMN comment.root_id IS '根评论ID(0=这是一条顶级评论)';
COMMENT ON COLUMN comment.reply_to_id IS '回复目标ID(0=直接回复根评论)';
COMMENT ON COLUMN comment.content IS '评论内容';
COMMENT ON COLUMN comment.reply_count IS '子回复数量';
...
-- 3. 【用户】我的评论历史
CREATE INDEX idx_comment_user ON comment(user_id, create_time DESC);
