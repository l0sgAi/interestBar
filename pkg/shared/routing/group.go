...
// gin/hertz，这样未来拆分微服务时，某领域包可以整体搬走而不带
// 框架包袱。
//
// 使用方式：
//  1. composition 层把 gin/hertz engine 包装成 RouterGroup 接口；
...
//
// 接收 appctx.AppContext，不暴露任何 gin/hertz 类型。
// composition 层负责把 *gin.Context 包装成 appctx.AppContext 后再调用。
type HandlerFunc func(c appctx.AppContext)

// RouterGroup 是路由组的抽象接口。
//
// 各领域的 RegisterRoutes 接收此接口，完成路由挂载。
// composition 层提供 gin 版实现（未来可加 hertz 版）。
