...
// 不更新旧行。频率限制统计排除 status=2（分类器跳过未产出回复，不占配额）。
type ReplyLog struct {
	...
	Status           int16      `json:"status" gorm:"column:status;type:smallint;not null"`      // 0=失败, 1=成功, 2=分类器跳过
...
type ReplyLogRepository interface {
	...
	// CountSinceByAgent 统计机器人自 since 以来的日志行数（含失败、排除 status=2，限频口径）。
	CountSinceByAgent(ctx context.Context, agentID uuid.UUID, since time.Time) (int64, error)
	// GetLastByAgent 取机器人最新一条日志（排除 status=2，算最小回复间隔）。无日志返回 nil, nil。
